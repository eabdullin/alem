...
function needsResolution(part: { url: string }): boolean {
  return (
    typeof part.url === "string" && part.url.startsWith(QURT_ATTACHMENT_PREFIX)
  );
}

function getAttachmentId(url: string): string {
  return url.slice(QURT_ATTACHMENT_PREFIX.length);
}

export interface QurtChatTransportOptions {
  /** Returns the agent to use. Called on each send so apiKey can be fetched fresh. */
  getAgent: () => Promise<Agent>;
  sendReasoning?: boolean;
  /** Resolves an attachment ID to base64 data. Called for file parts with url "qurt-attachment://<id>". */
 ...
 * "qurt-attachment://<id>" are resolved to data URLs via resolveAttachment.
 * Uses getAgent() so the agent can be created with the current apiKey on each send.
 */
export class QurtChatTransport implements ChatTransport<UIMessage> {
